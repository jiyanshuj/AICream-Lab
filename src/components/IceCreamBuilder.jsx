...
const SAUCE_COLORS = {
  'Hot Fudge': '#3d1a00', 'Caramel': '#c8860a',
  'Strawberry': '#e0405a', 'Blueberry': '#4a3b8c', 'Mango': '#f5a623',
}
...
const CSS = `
  ...
  /* real photo scoop */
  .rscoop{position:absolute;left:50%;border-radius:50%;overflow:hidden;cursor:pointer;
    animation:scoopDrop .7s cubic-bezier(.22,1.35,.36,1) both;transition:filter .2s,transform .2s;z-index:15;}
  ...
  /* glossy highlight */
  .rscoop::before{content:'';position:absolute;top:8%;left:14%;width:36%;height:26%;
    background:radial-gradient(ellipse,rgba(255,255,255,.62) 0%,rgba(255,255,255,0) 100%);
    border-radius:50%;transform:rotate(-30deg);pointer-events:none;z-index:20;}
  .rscoop::after{content:'';position:absolute;top:11%;left:17%;width:10%;height:7%;
    background:rgba(255,255,255,.88);border-radius:50%;pointer-events:none;z-index:20;}
  .scoop-pill{position:absolute;bottom:11%;left:50%;transform:translateX(-50%);
    background:rgba(255,255,255,.72);backdrop-filter:blur(8px);border-radius:50px;
    padding:3px 10px;font-size:10px;font-weight:800;color:#2d1b4e;white-space:nowrap;
  ...
  .cone-svg{position:absolute;bottom:0;left:50%;transform:translateX(-50%);z-index:5;
  ...
  /* toppings */
  .t-cloud{position:absolute;top:2%;left:50%;transform:translateX(-50%);z-index:30;
    display:flex;flex-wrap:wrap;justify-content:center;gap:3px;width:190px;pointer-events:none;}
  ...
  /* drop zones */
  .dzone{border:2px dashed #e2d4f0;border-radius:18px;padding:13px 16px;display:flex;align-items:center;gap:12px;
    transition:all .25s cubic-bezier(.34,1.2,.64,1);background:rgba(255,255,255,.7);backdrop-filter:blur(6px);}
  ...
  /* ingredient chips — real photo thumb for scoops */
  .ing-chip{display:flex;align-items:center;gap:7px;padding:8px 14px;border-radius:50px;
    font-size:12.5px;font-weight:700;cursor:grab;user-select:none;
    transition:all .2s;border:1.5px solid transparent;box-shadow:0 2px 8px rgba(0,0,0,.07);}
  ...
  .scoop-thumb{width:30px;height:30px;border-radius:50%;overflow:hidden;flex-shrink:0;
    border:2px solid rgba(255,255,255,.85);box-shadow:0 2px 7px rgba(0,0,0,.16);}
  ...
  .glass-card{background:rgba(255,255,255,.84);backdrop-filter:blur(16px);
    border:1px solid rgba(255,255,255,.95);border-radius:28px;
    box-shadow:0 8px 40px rgba(120,60,140,.08),0 2px 8px rgba(0,0,0,.04);}
  .tag3{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:50px;font-size:12px;font-weight:700;
    animation:tagIn .3s cubic-bezier(.34,1.4,.64,1) both;}
  ...
  .status3{background:linear-gradient(135deg,#fff0f5,#f5f0ff);border:1px solid #f0deff;border-radius:14px;
    padding:10px 18px;font-size:13px;font-weight:600;color:#6b3fa0;animation:stPop .3s ease-out;}
  ...
  .empty-hint{text-align:center;padding:60px 0 40px;color:#c0b0d0;font-size:13px;font-weight:600;pointer-events:none;}
`
...
function PhotoScoop({ scoop, index, onRemove }) {
  ...
  const imgSrc = SCOOP_IMAGES?.[scoop] || SCOOP_IMAGES?.default || `https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=300&h=300&fit=crop&q=80`
  return (
    <div className="rscoop"
      style={{
        width: size, height: size, bottom: bottom,
          ...
          <div className="glass-card" style={{ padding:'28px 20px 22px', marginBottom:20 }}>
            <div className="bld-scene">
              <div className="bld-stage">
                ...
                {layers.length === 0
                  ? <div className="empty-hint">✨ Add your first scoop!</div>
                  : layers.map((scoop,i) => (
            ...
            <div className="summ-sec">
              ...
              {sauces.length>0 && <div className="summ-row"><span>🍯</span><div className="tlist">{sauces.map((s,i)=><span key={i} className="tag3 tag-sa">{s}<button className="tag-rm" onClick={()=>removeItem('sauce',i)}>×</button></span>)}</div></div>}
              {toppings.length>0 && <div className="summ-row"><span>🎉</span><div className="tlist">{toppings.map((t,i)=><span key={i} className="tag3 tag-tp">{t}<button className="tag-rm" onClick={()=>removeItem('topping',i)}>×</button></span>)}</div></div>}
            </div>
          </div>
            ...
            {(SCOOPS||[]).map(s => (
              <div key={s.name} className="ing-chip chip-scoop"
                ...
                <div className="scoop-thumb">
                  <img src={SCOOP_IMAGES?.[s.name]||`https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=60&h=60&fit=crop&q=80`} alt={s.name} loading="lazy"/>
                </div>
                {s.name}
              </div>
            ))}
